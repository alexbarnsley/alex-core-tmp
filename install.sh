...
alias ark="$HOME/core-tacochain/packages/core/bin/run"
echo 'alias taco-chain="$HOME/core-tacochain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-tacochain"
git clone "git@github.com:alexbarnsley/alex-core-tmp.git" "$HOME/core-tacochain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'git@github.com:alexbarnsley/alex-core-tmp.git'"

    exit 1
fi

cd "$HOME/core-tacochain"
...
rm -rf "$HOME/.config/@taco"
rm -rf "$HOME/.config/@taco-chain"
rm -rf "$HOME/.config/taco-chain-core"
